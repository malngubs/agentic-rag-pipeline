...
class LLMManager:
    ...
    def _initialize_models(self):
        """Initialize available models - OpenAI primary, Ollama fallback"""
        self.available_models = [
            # PRIMARY: OpenAI GPT-4 Mini (faster, more capable)
            {"name": "gpt-4o-mini", "provider": "openai", "max_tokens": 2048, "temperature": 0.7},
            # FALLBACK: Ollama (local, always available)
            {"name": "llama3.2", "provider": "ollama", "max_tokens": 2048, "temperature": 0.7}
        ]
        self.logger.info(f"✅ Models initialized - Primary: OpenAI GPT-4 Mini, Fallback: Ollama")
